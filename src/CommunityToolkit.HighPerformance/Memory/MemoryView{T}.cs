...
/// <summary>
/// <see cref="MemoryView{T}"/> represents a discontinuous typed view over a <see cref="Memory{T}"/> of bytes,
/// similar to <see cref="SpanView{T}"/>. Unlike <see cref="SpanView{T}"/>, it is not a byref-like type.
/// </summary>
/// <typeparam name="T">
/// The type of elements in the <see cref="MemoryView{T}"/>.
/// Elements must be unmanaged types (primitive or blittable).
/// </typeparam>
/// <remarks>
/// <see cref="MemoryView{T}"/> represents a view over a memory region, from a specified
/// offset onwards, of a sequence of consecutive --although separated by a stride-- typed elements.
/// <list type="bullet">
/// <item>
/// If the stride is the size of the elements' type, it behaves like a regular <see cref="Memory{T}"/>.
/// </item>
/// <item>
/// <para>
...
/// </para>
/// <para>
/// For example, to view as a <c>MemoryView&lt;Vector3&gt;</c> only the
/// <c>Position</c> fields of a bigger <c>Transform</c> structure (composed of position, rotation,
/// and scale) from a <c>Memory&lt;Transform&gt;</c>.
/// </para>
/// </item>
/// </list>
/// </remarks>
    ...
    where T : unmanaged
{
    ...
    /// <summary>
    /// Creates a new <see cref="MemoryView{T}"/> over the specified <see cref="Memory{T}"/> of bytes
    /// beginning at <paramref name="offset"/> and with a specific <paramref name="stride"/>.
    /// </summary>
    /// <param name="memory">The target <see cref="Memory{T}"/> of bytes to wrap.</param>
    ...
    /// </exception>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private unsafe MemoryView(Memory<byte> memory, int offset, int stride, int length)
    {
        if ((uint)offset > (uint)memory.Length)
        {
            ThrowHelper.ThrowArgumentOutOfRangeExceptionForOffset();
        ...
        Memory<byte> memoryFromOffset = memory.Slice(start: offset);
    ...
    /// <summary>
    /// Creates a new <see cref="MemoryView{T}"/> instance over a buffer, representing the values
    ...
    /// <param name="buffer">
    /// The <see cref="Memory{T}"/> of <typeparamref name="TBuffer"/> over which to create a view.
    /// </param>
    /// <param name="field">
    ...
    /// </param>
    /// <returns>A <see cref="MemoryView{T}"/> instance with the specified parameters.</returns>
    /// <remarks>
    /// <para>
    /// The <see cref="DangerousCreate{TBuffer}(Memory{TBuffer}, ref T)"/> method creates a view over a buffer of elements
    /// of type <typeparamref name="TBuffer"/> as if you had a <see cref="Memory{T}"/> of <typeparamref name="T"/>,
    ...
    {
        Memory<byte> memory = buffer.Cast<TBuffer, byte>();
        nint offset = Unsafe.ByteOffset(ref MemoryMarshal.GetReference(memory.Span), ref Unsafe.As<T, byte>(ref field));
    ...
    /// <remarks>
    /// <para>
    /// The <see cref="DangerousCreate{TBuffer}(Memory{TBuffer}, int)"/> method creates a view over a buffer of elements
    /// of type <typeparamref name="TBuffer"/> as if you had a <see cref="Memory{T}"/> of <typeparamref name="T"/>,
    ...
    /// <summary>
    /// Gets an empty <see cref="MemoryView{T}"/>.
    /// </summary>
    ...
    /// <summary>
    /// Gets a <see cref="SpanView{T}"/> from the memory view.
    /// </summary>
    public SpanView<T> SpanView
    {
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    /// <returns>The sliced <see cref="MemoryView{T}"/>.</returns>
    /// <exception cref="ArgumentOutOfRangeException">
    ...
    public MemoryView<T> Slice(int start)
    {
        ...
        return new MemoryView<T>(this.memory, start * Stride, Stride, (int)(Length - start));
    }
    ...
    public MemoryView<T> Slice(int start, int length)
    {
        ...
        Memory<byte> slicedMemory = this.memory.Slice(start: 0, byteLength);

        return new MemoryView<T>(slicedMemory, offset: start * Stride, Stride, length);
    }
    ...
    /// <summary>
    /// Defines an implicit conversion of a <see cref="Memory{T}"/> to a <see cref="MemoryView{T}"/>.
    /// </summary>
    ...
    /// <summary>
    /// For <see cref="MemoryView{Char}"/>, returns a new instance of string that represents
    ...
    public override string ToString()
    {
        ...
        return nameof(MemoryView<T>) + $"<{typeof(T).Name}>[{Length}]";
    }
}