...
#pragma warning disable 0809 // Obsolete member 'ReadOnlySpanView<T>.Equals(object)' overrides non-obsolete member 'object.Equals(object)'

namespace CommunityToolkit.HighPerformance;

/// <summary>
/// <see cref="ReadOnlySpanView{T}"/> represents a discontinuous typed read-only view
/// over a <see cref="ReadOnlySpan{T}"/> of bytes.
/// </summary>
/// <typeparam name="T">
/// The type of elements in the <see cref="ReadOnlySpanView{T}"/>.
/// Elements must be unmanaged types (primitive or blittable).
/// </typeparam>
/// <remarks>
/// <see cref="ReadOnlySpanView{T}"/> represents a read-only view over a memory region, from a specified
/// offset onwards, of a sequence of consecutive --although separated by a stride-- typed elements.
/// <list type="bullet">
/// <item>
/// If the stride is the size of the elements' type, it behaves like a regular <see cref="ReadOnlySpan{T}"/>.
/// </item>
/// <item>
/// <para>
...
/// </para>
/// <para>
/// For example, to view as a <c>ReadOnlySpanView&lt;Vector3&gt;</c> only the
/// <c>Position</c> fields of a bigger <c>Transform</c> structure (composed of position, rotation,
/// and scale) from a <c>ReadOnlySpan&lt;Transform&gt;</c>.
/// </para>
/// </item>
/// </list>
/// </remarks>
    ...
    where T : unmanaged
{
    ...
    /// <summary>
    /// Creates a new <see cref="ReadOnlySpanView{T}"/> from a specified number of <typeparamref name="T"/> elements
    ...
    public unsafe ReadOnlySpanView(void* pointer, int stride, int length)
    {
        ...
        this.span = new ReadOnlySpan<byte>(pointer, MemoryViewHelper.GetByteLength<T>(stride, length));
        Stride = stride;
        Length = length;
    }

    /// <summary>
    /// Creates a new <see cref="ReadOnlySpanView{T}"/> over the specified <see cref="ReadOnlySpan{T}"/> of bytes
    ...
    internal unsafe ReadOnlySpanView(ReadOnlySpan<byte> span, int offset, int stride, int length)
    {
        ...
        ReadOnlySpan<byte> spanFromOffset = span.Slice(offset);
    ...
    /// <summary>
    /// Creates a new <see cref="ReadOnlySpanView{T}"/> instance over a span, representing the values
    /// at a given offset from each element.
    /// </summary>
    /// <typeparam name="TBuffer">The type of the elements in the span to wrap.</typeparam>
    /// <param name="buffer">
    /// The <see cref="ReadOnlySpan{T}"/> of <typeparamref name="TBuffer"/> over which to create a view.
    /// </param>
    /// <param name="field">
    ...
    /// </param>
    /// <returns>A <see cref="ReadOnlySpanView{T}"/> instance with the specified parameters.</returns>
    /// <remarks>
    /// <para>
    /// The <see cref="DangerousCreate{TBuffer}(ReadOnlySpan{TBuffer}, ref readonly T)"/> method creates a view over
    /// a span of elements of type <typeparamref name="TBuffer"/> as if you had a <see cref="ReadOnlySpan{T}"/>
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe ReadOnlySpanView<T> DangerousCreate<TBuffer>(ReadOnlySpan<TBuffer> buffer, ref readonly T field)
        where TBuffer : unmanaged
    {
        ReadOnlySpan<byte> span = MemoryMarshal.AsBytes(buffer);
        IntPtr offset = Unsafe.ByteOffset(ref MemoryMarshal.GetReference(span), ref Unsafe.As<T, byte>(ref Unsafe.AsRef(in field)));
    ...
    /// <remarks>
    /// <para>
    /// The <see cref="DangerousCreate{TBuffer}(ReadOnlySpan{TBuffer}, int)"/> method creates a view over a buffer
    /// of elements of type <typeparamref name="TBuffer"/> as if you had a <see cref="ReadOnlySpan{T}"/>
    /// of <typeparamref name="T"/>, but only exposes the field at the specified <paramref name="offset"/>
    /// within each element in <paramref name="buffer"/>.
    /// </para>
    /// <para>
    ...
    /// </summary>
    internal ReadOnlySpan<byte> Span => this.span;

    /// <summary>
    /// Gets an empty <see cref="ReadOnlySpanView{T}"/>.
    /// </summary>
    ...
    /// <returns>The element from the read-only span view.</returns>
    ...
    /// <summary>
    /// Gets an enumerator for this <see cref="ReadOnlySpanView{T}"/>.
    /// </summary>
    /// <returns>
    ...
    /// <returns>The sliced <see cref="ReadOnlySpanView{T}"/>.</returns>
    /// <exception cref="ArgumentOutOfRangeException">
    ...
    public ReadOnlySpanView<T> Slice(int start)
    {
        ...
        return new ReadOnlySpanView<T>(this.span, start * Stride, Stride, (int)(Length - start));
    }
    ...
    public ReadOnlySpanView<T> Slice(int start, int length)
    {
        ...
        ReadOnlySpan<byte> slicedSpan = this.span.Slice(start: 0, byteLength);

        return new ReadOnlySpanView<T>(slicedSpan, offset: start * Stride, Stride, length);
    }
    ...
    public void CopyTo(SpanView<T> destination)
    {
        ...
        MemoryViewHelper.CopyTo(sourceRef: ref Unsafe.AsRef(in DangerousGetReference()),
    ...
    /// <summary>
    /// Returns a reference to the first element of the <see cref="ReadOnlySpanView{T}"/>.
    ...
    /// <summary>
    /// Determines whether the specified <see cref="ReadOnlySpanView{T}"/> is equal to the current span view.
    /// </summary>
    /// <param name="other">The <see cref="ReadOnlySpanView{T}"/> to compare against.</param>
    /// <returns>
    ...
    /// </exception>
    [Obsolete("Equals() on ReadOnlySpanView will always throw an exception. Use the equality operator instead.")]
    [EditorBrowsable(EditorBrowsableState.Never)]
...
#pragma warning restore CA1065 // Do not raise exceptions in unexpected locations
    ...
    /// <summary>
    /// Determines whether two <see cref="ReadOnlySpanView{T}"/> instances are equal.
    /// </summary>
    /// <param name="left">The <see cref="ReadOnlySpanView{T}"/> to the left of the operator.</param>
    /// <param name="right">The <see cref="ReadOnlySpanView{T}"/> to the right of the operator.</param>
    /// <returns>
    /// Returns <see langword="true"/> if both span views point at the same memory, have the same length,
    /// and the same stride.
    /// Note that this does <strong>not</strong> check to see if the <strong>contents</strong> are equal.
    ...
    /// <summary>
    /// Determines whether two <see cref="ReadOnlySpanView{T}"/> instances are not equal.
    /// </summary>
    /// <param name="left">The <see cref="ReadOnlySpanView{T}"/> to the left of the operator.</param>
    /// <param name="right">The <see cref="ReadOnlySpanView{T}"/> to the right of the operator.</param>
    /// <returns>
    ...
    /// Always thrown by this method.
    /// </exception>
    [Obsolete("GetHashCode() on ReadOnlySpanView will always throw an exception.")]
    [EditorBrowsable(EditorBrowsableState.Never)]
...
#pragma warning restore CA1065 // Do not raise exceptions in unexpected locations
    ...
    /// <summary>
    /// For <see cref="ReadOnlySpanView{Char}"/>, returns a new instance of <see cref="string"/> that represents
    ...
    public override unsafe string ToString()
    {
        ...
        return nameof(ReadOnlySpanView<T>) + $"<{typeof(T).Name}>[{Length}]";
    }
    ...
    /// <summary>
    /// Defines an implicit conversion of a <see cref="SpanView{T}"/> to a <see cref="ReadOnlySpanView{T}"/>.
    /// </summary>
    public static implicit operator ReadOnlySpanView<T>(SpanView<T> view)
    {
        return new ReadOnlySpanView<T>(view.Span, 0, view.Stride, (int)view.Length);
    }

    /// <summary>
    /// Defines an implicit conversion of a <see cref="ReadOnlySpan{T}"/> to a <see cref="ReadOnlySpanView{T}"/>.
    /// </summary>
    ...
    /// <summary>
    /// Defines an implicit conversion of a <see cref="Span{T}"/> to a <see cref="ReadOnlySpanView{T}"/>.
    /// </summary>
    ...
    /// <summary>
    /// Enumerates the elements of a <see cref="ReadOnlySpanView{T}"/>.
    /// </summary>
    public ref struct Enumerator : IEnumerator<T>
    {
        ...
        /// </summary>
        private readonly ReadOnlySpanView<T> view;
        ...
        /// <summary>
        /// Advances the enumerator to the next element of the <see cref="ReadOnlySpanView{T}"/>.
        /// </summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
